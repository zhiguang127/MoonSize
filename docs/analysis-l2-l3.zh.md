...
L2 / L3 不实现 retained size、自动优化或源码映射。`check_budget` 和既有 CLI 预算参数始终基于原始文件字节，不依赖匹配覆盖率或引用分析是否完整。v0.3 的宿主扩展另增独立压缩指标和预算，不修改上述核心归因口径，见 [CI 决策接口](engineering-ci.zh.md)。
