...
export function renderBody(result, files = {}, requestedLocale='en') {
  ...
  <div class="metric"><div class="eyebrow">${copy.sizeBudget}</div><strong class="${budget ? budget.passed ? 'down' : 'up' : ''}">${budget ? budget.passed ? copy.pass : copy.fail : copy.notSet}</strong><span class="muted small">${budget ? `${copy.total} ≤ ${budget.max_bytes === null ? copy.unlimited : formatBytes(budget.max_bytes)} · ${copy.growth} ≤ ${budget.max_growth_bytes === null ? copy.unlimited : formatBytes(budget.max_growth_bytes)}` : copy.budgetHint}</span></div></div>
